...
<c:if test="<%= ldapImportConfiguration.importUserPasswordAutogenerated() %>">
	<aui:script>
		function <portlet:namespace />enableExport() {
			var exportCheckbox = document.querySelector('#<portlet:namespace />ldapExportEnabled');
			var importCheckbox = document.querySelector('#<portlet:namespace />ldapImportEnabled');
...
<aui:script>
	...
	function <portlet:namespace />saveLdap() {
		var ldapServerIds = document.querySelector('.ldap-servers .table-data tr').map(
			function(index, item) {
				return  document.querySelector(item).data('ldapserverid');
			}
		).get();

		document.querySelector(document.<portlet:namespace />fm).fm('ldap--<%= LDAPConstants.AUTH_SERVER_PRIORITY %>--').val(ldapServerIds.join(','));
	}
