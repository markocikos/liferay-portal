...
test(
	...
	async ({fdsSamplePage, page}) => {
		...
		await test.step('Create a custom views and set it as the default one', async () => {
			...
			await expect(actionsDropdown.getByRole('menuitem')).toHaveText(
				'Save View As...'
			);
		...
		await test.step('Edit custom view, by changing visibility of one column', async () => {
			...
			actionsDropdown.getByRole('menuitem', {
				name: 'Save View',
			});
		...
		await test.step('Can change a custom view name', async () => {
			...
			await expect(
				actionsDropdown.getByRole('menuitem', {name: 'Rename View'})
			).toBeVisible();
		...
		await test.step('Delete a custom view', async () => {
			...
			await expect(
				actionsDropdown.getByRole('menuitem', {name: 'Delete View'})
			).toBeVisible();
