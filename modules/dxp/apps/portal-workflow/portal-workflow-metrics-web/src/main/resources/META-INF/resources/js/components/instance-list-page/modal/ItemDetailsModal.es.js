...
const ItemDetailsModal = ({closeItemDetailsModal, processId, visible}) => {
		...
		<>
			{visible && (
				<ClayModal observer={observer} size="lg">
					<ClayModal.Header>
						<div className="font-weight-medium modal-title">
							<span
...
ItemDetailsModal.SectionTitle = ({children, className = ''}) => {
	const classNames = `${className} font-weight-medium mb-4`;

	return <h4 className={classNames}>{children}</h4>;
};

ItemDetailsModal.SectionSubTitle = ({children}) => {
	return (
...
ItemDetailsModal.SectionAttribute = ({description, detail}) => {
	return (
		<p className="row">
